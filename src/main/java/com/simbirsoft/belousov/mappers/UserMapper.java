...
public abstract class UserMapper {
    ...
    @Mappings({
    @Mapping(target = "roleId", expression = "java(roleRepository.findById(userRequestDto.getRoleId()).orElseThrow(() -> new NoSuchException(\"Задача не найдена\")))")
    })
    public abstract UserEntity userRequestDtoToEntity(UserRequestDto userRequestDto) throws NoSuchException;
    @Mappings({
    @Mapping(target = "releaseId", expression = "java(userEntity.getRole().getRoleId())")
    })
